  ...
  try {
    ...
    if (!Array.isArray(rawQuestions)) {
      ...
      if (rawQuestions.questions) {
        console.log("📌 Found questions in .questions property");
        rawQuestions = rawQuestions.questions;
      } else if (rawQuestions.data) {
    ...
    // Map Legacy Questions to Frontend Format with INTELLIGENT HANDLING
    const questions = rawQuestions.map((q, qIndex) => {
      ...
      // If answer is not PLACEHOLDER, try to match it
      if (safeAnswer && safeAnswer !== "PLACEHOLDER" && safeAnswer !== "") {
        // Try exact match
        correctIndex = safeOptions.findIndex(opt => opt === safeAnswer);
      ...
      // Final fallback to first option if no match found
      if (correctIndex === -1 || correctIndex === undefined) {
        correctIndex = 0;
        console.log(`⚠️ Question ${qIndex + 1}: Using first option as default. Answer was: "${safeAnswer}"`);
      } else {
        console.log(`✅ Question ${qIndex + 1}: Matched answer to option ${correctIndex}: "${safeOptions[correctIndex]}"`);
      }

      // Ensure we have at least 2 options
      if (safeOptions.length < 2) {
        console.warn(`⚠️ Question ${qIndex + 1} has less than 2 options, padding with defaults`);
      ...
      return {
        ...
        type: "multiple-choice",
        explanation: q.explanation || "Generated by Quizito AI from your PDF",
