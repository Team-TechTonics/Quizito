...
app.post("/api/quiz/generate-from-pdf", pdfUpload.single("file"), async (req, res) => {
  try {
    ...
    // Map Legacy Questions to Frontend Format if needed
    // app2.py returns: { question, options, answer (string) }
    // Frontend expects: { question, options, correctAnswer (index), explanation? }
...
app.post("/api/quiz-generation/from-audio", audioUpload.single("audio"), async (req, res) => {
  try {
    ...
    // Convert text → Quiz using Python AI Service (Legacy app2.py)
    console.log("🚀 Sending transcription to Python AI for generation...");

    // Construct Python URL
    let pythonUrl = process.env.PYTHON_SERVICE_URL || "https://quizito-ndjq.onrender.com";
    if (pythonUrl.endsWith('/')) {
      pythonUrl = pythonUrl.slice(0, -1);
    }
    ...
    // Create a virtual file for the text content
    const FormData = require("form-data");
    const audioForm = new FormData();
    audioForm.append("file", Buffer.from(text, 'utf-8'), {
    ...
    // Map Legacy Questions (copy of logic above)
    let rawAudioQuestions = pythonResponse.data;
    if (!Array.isArray(rawAudioQuestions)) {
      if (rawAudioQuestions.questions) rawAudioQuestions = rawAudioQuestions.questions;
      else if (rawAudioQuestions.data) rawAudioQuestions = rawAudioQuestions.data;
      else rawAudioQuestions = [];
    }

    const audioQuestions = rawAudioQuestions.map(q => {
      let correctIndex = q.options.indexOf(q.answer);
      ...
      return {
        ...
        correctAnswer: correctIndex,
        type: "multiple-choice",
        explanation: "Generated from Audio"
      };
    });
