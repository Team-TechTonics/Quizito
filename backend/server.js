...
app.get("/api/classes", authenticate, async (req, res) => {
  try {
    const classes = await Class.find({ teacher: req.user._id })
      .populate("students.student", "displayName username email avatar");
    // Count students
...
app.get("/api/classes/:id", authenticate, async (req, res) => {
  try {
    const cls = await Class.findById(req.params.id)
      .populate("students.student", "displayName username email avatar lastActive stats");
    if (!cls) return res.status(404).json({ success: false, message: "Class not found" });
    if (cls.teacher.toString() !== req.user._id.toString() && !req.user.role === 'admin') {
      return res.status(403).json({ success: false, message: "Not authorized" });
    }

    // Transform students list for frontend
    const studentsList = cls.students.map(s => {
      if (!s.student) return null;
      return {
        id: s.student._id,
        name: s.student.displayName || s.student.username,
        email: s.student.email,
        avatar: s.student.avatar,
        lastActive: s.student.lastActive ? new Date(s.student.lastActive).toLocaleDateString() : 'Never',
        progress: Math.min(100, Math.floor((s.student.stats?.experience || 0) / 100)) // Mock progress logic
      };
    }).filter(Boolean);

    res.json({ success: true, class: cls, students: studentsList });
  } catch (error) {
    logger.error("Get class detail error:", error);
...
app.delete("/api/classes/:id", authenticate, async (req, res) => {
  try {
    const cls = await Class.findOneAndDelete({ _id: req.params.id, teacher: req.user._id });
    if (!cls) return res.status(404).json({ success: false, message: "Class not found or unauthorized" });
...
// Invite student (by email) - Simplified logic
app.post("/api/classes/:id/invite", authenticate, async (req, res) => {
  try {
    ...
    if (cls.students.some(s => s.student.toString() === userToInvite._id.toString())) {
      return res.status(400).json({ success: false, message: "Student already enrolled" });
    }

    cls.students.push({ student: userToInvite._id });
    await cls.save();
...
// Assignments
app.post("/api/classes/:id/assignments", authenticate, async (req, res) => {
  try {
    const { quizId, dueDate, settings } = req.body;
    const cls = await Class.findOne({ _id: req.params.id, teacher: req.user._id });
    if (!cls) return res.status(404).json({ success: false, message: "Class not found" });
    ...
    await cls.save();
    res.json({ success: true, message: "Quiz assigned" });
  } catch (error) {
    logger.error("Assign quiz error:", error);
    res.status(500).json({ success: false, message: "Failed to assign quiz" });
  }
});

app.get("/api/classes/:id/assignments", authenticate, async (req, res) => {
  try {
    const cls = await Class.findById(req.params.id).populate("assignments.quiz", "title questions");
    if (!cls) return res.status(404).json({ success: false, message: "Class not found" });

    res.json({ success: true, assignments: cls.assignments });
  } catch (error) {
...
app.post("/api/classes/join", authenticate, async (req, res) => {
  try {
    ...
    if (cls.students.some(s => s.student.toString() === req.user._id.toString())) {
      return res.status(400).json({ success: false, message: "Already joined this class" });
    }

    cls.students.push({ student: req.user._id });
    await cls.save();
    res.json({ success: true, message: "Joined class successfully", classId: cls._id });
  } catch (error) {
...
// Get Student Assignments (All classes)
app.get("/api/classes/student/assignments", authenticate, async (req, res) => {
  try {
    // Find classes where student is enrolled
    const classes = await Class.find({ "students.student": req.user._id })
      .populate("assignments.quiz", "title description difficulty questions")
      .select("name subject assignments");

    // Aggregate assignments
    let allAssignments = [];
    classes.forEach(cls => {
      if (cls.assignments && cls.assignments.length > 0) {
        const classAssignments = cls.assignments.map(a => ({
          ...a.toObject(),
          className: cls.name,
          subject: cls.subject,
          classId: cls._id
